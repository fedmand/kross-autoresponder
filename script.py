...
import prompt_store  # shared system_prompt.md path + read/write (also used by web app)
...
def build_system_prompt(apartment_name, apartment_info, reservation):
    ...
    # Read fresh every call (like load_apartment_file) so edits made from the web
    # app take effect on the next poll cycle with no restart. If the file is
    # missing, this raises and propagates to the caller's per-thread error
    # handler, which logs it and alerts the host via Telegram — no silent
    # fallback to a baked-in default.
    system_prompt_base = prompt_store.read_prompt()
